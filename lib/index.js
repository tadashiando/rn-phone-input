...
export default class PhoneInput extends Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      ...
      countryCodeLength: countryData ? `+${countryData.dialCode}`.length : 0,
      value: null,
      inputValue: "",
    };
  }
  ...
  componentDidMount() {
    ...
    if (this.props.textProps.placeholder) {
      this.setState({ formattedNumber: this.state.formattedNumber + ' ' + this.props.textProps.placeholder});
    }
  }
      ...
      <View style={[styles.container, this.props.style]}>
        ...
        <View style={{ flex: 1, marginLeft: this.props.offset || 10 }}>
          <TextComponent
            ...
            onBlur={() => {
              if (this.state.formattedNumber.replace(/[^0-9+-]/g, '').length == this.state.countryCodeLength) {
                this.setState({ formattedNumber: formattedNumber + ' ' + this.props.textProps.placeholder});
              }
            }}
